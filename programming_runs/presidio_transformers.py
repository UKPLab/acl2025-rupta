...
from presidio_trans_config import BERT_DEID_CONFIGURATION
...
if __name__ == "__main__":
    ...
    data = []
    with open('./benchmarks/Wiki_People/test_sampled3.jsonl') as f:
        raw = f.readlines()
        for r in raw:
            data.append(json.loads(r))
    with open('./root/test_reflexion/presidio_roberta_new.txt', 'w') as f:
        for d in data:
            results = analyzer.analyze(text=d['text'],
                                       language='en')
            anonymized_text = anonymizer.anonymize(text=d['text'], analyzer_results=results).text
