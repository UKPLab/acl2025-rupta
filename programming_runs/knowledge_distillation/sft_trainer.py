...
if data_format == 'Instruction':
    ...
    train_data_format = train_data_raw
    val_data_format = val_data_raw
else:
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['input'])):
        text = f"Please anonymize the biography:\n\n{example['input'][i]}\n\nAnonymization result: {example['output'][i]}"
...
if model_name == "google--flan-t5-large--hf":
    ...
    tokenized_train_datasets = tokenized_train_datasets.remove_columns(['output', 'input', 'people', 'label'])
    tokenized_val_datasets = val_dataset.map(tokenize_function, batched=True)
    tokenized_val_datasets = tokenized_val_datasets.remove_columns(['output', 'input', 'people', 'label'])
