...
from .generator_utils import (generic_generate_func_impl, generic_generate_internal_tests,
                              generic_generate_self_reflection, generic_detection, generic_rewriting,
...
DETECTION_INSTRUCTION = ("Given the input description text, first answer the name of the"
                                    " person described by the text, then identify every entities "
                                    ...
                                    "are removed the resulting text is anonymised. "
                                    "Return these entities as a JSONformatted list where there are two fields for "
                                    "your two answers \"people\" and \"sensitive entities\"")
GENERAL_SYSTEM_INSTRUCTION = ("You are an efficient assistant. "
                              "Keep your responses short and provide no explanations.")
WHOLE_TASK_INSTRUCTION = ("You are a text anonymization annotator. "
                          ...
                          "First, detection, you need to detect all the sensitive entities. "
                          "Second, rewriting, replace sensitive entities with more general entities to make "
...
SIMPLE_REWRITING_INSTRUCTION = "Refine your rewriting according to the feedbacks to improve the evaluation score."
REFELECTION_REWRITING_INSTRUCTION = ("Rewrite the description text. Format your response as shown below."
                                     "\n[Rewriting results]:\n")
PRIVACY_REFLECTION_INSTRUCTION = ("You are very knowledgeable about celebrities. Given the description text"
                                  " and the name of person, respond with the confidence score "
                                  "about associating the description text with the given person, 0 represents fully "
                                  "irrelevant and 100 represents fully relevant, then give me advice about "
                                  "which part of the description text can be used to infer the "
                                  "person and how to further generalize the text to decrease the confidence score."
                                  "Format your response as follows:\nConfidence score:\nAdvice:\n")
UTILITY_REFLECTION_INSTRUCTION = ("You are a utility evaluator of the anonymized text. Given the original text, "
                                  "the anonymized text and the classification label of the original text,"
                                  " your task is to first give me the confidence score of classifying the"
                                  ...
                                  " how to recover some information from the original text to the anonymized text to "
...
class PyGenerator(Generator):
    ...
    def rewrite(
        ...
        return generic_rewriting(
            ...
            detection_result_prefix=DETECTION_RESULT_PREFIX,
    ...
    def privacy_reflex(self, model: ModelBase, rewriting, people):
        return generic_privacy_reflection(
            ...
            people=people,
            privacy_reflection_chat_instruction=PRIVACY_REFLECTION_INSTRUCTION,
            privacy_reflection_completion_instruction=PRIVACY_REFLECTION_INSTRUCTION,
        )
    ...
    def utility_reflex(self, input_text: str, model: ModelBase, rewriting, label):
        return generic_utility_reflection(
            ...
            label=label,
            curr_rewriting=rewriting,
            utility_reflection_chat_instruction=UTILITY_REFLECTION_INSTRUCTION,
            utility_reflection_completion_instruction=UTILITY_REFLECTION_INSTRUCTION,
        )
